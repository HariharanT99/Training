...
SELECT TOP 1 C.CategoryName,COUNT(P.ProductID) AS Products FROM Products P
JOIN Categories C ON P.CategoryID = C.CategoryID
...
/* 3.	Which category has no product? */ --25 mins
...
/* 5.	Which category has lot of products with respect to quantity on hand? */
...
SELECT P.CategoryID, MAX(P.UnitPrice) AS HighPrice FROM Products P
GROUP BY P.CategoryID;


/* 7.	Category wise display the product with minimum quantity on hand? */
